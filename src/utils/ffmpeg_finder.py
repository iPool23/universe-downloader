...
from src.config import FFMPEG_LOCATIONS, FFMPEG_EXECUTABLE_NAMES
...
def find_ffmpeg() -> Optional[str]:
    ...
    for executable_name in FFMPEG_EXECUTABLE_NAMES:
        which_ffmpeg = shutil.which(executable_name)
        if which_ffmpeg:
    ...
    for location in FFMPEG_LOCATIONS:
        for executable_name in FFMPEG_EXECUTABLE_NAMES:
            executable_path = os.path.join(location, executable_name)
